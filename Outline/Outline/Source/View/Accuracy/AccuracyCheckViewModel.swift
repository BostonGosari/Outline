...
class AccuracyCheckViewModel: ObservableObject {
    @Published var gpsCourses = GPSArtCourse()
    ...
    private let courseId = "2B0F18CF-57BC-4695-B575-B0F9C76E3424"
    
    func getCourses() {
        courseModel.readCourse(id: courseId) { res in
            switch res {
            case .success(let gpsArtCourse):
                self.gpsCourses = gpsArtCourse
                print("success to load gpsArt")
            case .failure(let failure):
                print("fail to load gpsArt \(failure)")
            }
        }
    }
    func resetRunning() {
